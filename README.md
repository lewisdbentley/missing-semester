...
``` journalctl -b -3```to access the last three boots. Next, log their unique lines individually into a file. ```sed -E "s/^.*palicomp\s(.*)$/\1/" | sort | uniq > three-last-boots``` for each one (N.B, take everything after the timestamp using sed, as this always varies. Note that ```sort``` must precede ```uniq```. Next, sort and count all these lines. ```cat three-last-boots | sort | uniq -c | sort -n | sed 's/^\s*//' > three-last-boots-counted``` (Also uses sed to remove preceding whitespace generated by ```uniq -c```) Finally, use sed to elimate any lines whose count is 3 (since it was shared among all the boots). ```cat three-last-boots-counted | sed '/^3/d' > three-last-boots-no-shared-message```
- (6) Find an online data set.
- fetch using curl ```url="https://ucr.fbi.gov/crime-in-the-u.s/2016/crime-in-the-u.s.-2016/topic-pages/tables/table-1" | curl -s "$url"``` Now, use pup to extract out a column of the data. ```pup 'div#table-data-container:nth-of-type(3) td.group1 text{}'``` Also, remove any empty lines. ```rg -v -e '^$' | sed "s/,//g"``` (And the comma thousand separators). Finally, pipe this to the stats command written in awk ``` | stats ```.
- ```population=$(curl -s https://... | pup ...``` creates an intermediate variable to avoid having a long unreadable pipeline.
